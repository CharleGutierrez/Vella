...
async function writeScaffoldToDisk(folderName: string, fileName: string, content: string) {
...
`;
    ...
    let nuclearCoreSimulatorDisposable = vscode.commands.registerCommand('vella.nuclearCoreSimulator', async () => {
        const scenario = await vscode.window.showQuickPick(
            ['Normal Operation (100% Power)', 'SCRAM (Emergency Control Rod Drop)', 'Xenon Poisoning (Reactivity Decay)'],
            { placeHolder: 'Select Reactor Core scenario to simulate via UDP Telemetry:' }
        );
    ...
    let scadaAnomalyInjectorDisposable = vscode.commands.registerCommand('vella.scadaAnomalyInjector', async () => {
        const scenario = await vscode.window.showQuickPick(
            ['Pipeline Pressure Blowout (Critical Spike)', 'Pump Cavitation (Vibration Anomaly)', 'Cooling System Failure (Thermal Runaway)'],
            { placeHolder: 'Select Industrial Anomaly to Inject into Vella Engine:' }
        );
    ...
    let f1UdpReplayerDisposable = vscode.commands.registerCommand('vella.f1UdpReplayer', async () => {
        const track = await vscode.window.showQuickPick(
            ['Silverstone Circuit - High Speed Straight', 'Monaco Grand Prix - Hairpin Braking Zone', 'Spa-Francorchamps - Eau Rouge'],
            { placeHolder: 'Select F1 telemetry scenario to inject via UDP:' }
        );
        ...
        const interval = setInterval(() => {
            ...
            client.send(payload, 8081, 'localhost', (err: any) => {
                if (err) console.error("F1 UDP Error:", err);
            });
    ...
    let generateWeb3BindingsDisposable = vscode.commands.registerCommand('vella.generateWeb3Bindings', async () => {
        ...
        for (const item of abiContent) {
            if (item.type === 'function') {
                const inputs = (item.inputs || []).map((i: any, idx: number) => `${i.name || 'arg' + idx}: ${i.type.includes('int') ? 'number | bigint' : 'string'}`).join(', ');
                const outputs = (item.outputs || []).map((o: any) => o.type.includes('int') ? 'bigint' : 'string').join(' | ');
        ...
        for (const file of files) {
            ...
            while ((match = structRegex.exec(text)) !== null) {
                ...
                while ((fieldMatch = fieldRegex.exec(fieldsRaw)) !== null) {
                    ...
                    let tsType = "any";
                    if (rustType.includes("String") || rustType.includes("str")) tsType = "string";
                    else if (rustType.includes("i32") || rustType.includes("f64") || rustType.includes("u64")) tsType = "number";
                    else if (rustType.includes("bool")) tsType = "boolean";
                    else if (rustType.includes("Vec")) {
                        if (rustType.includes("String")) tsType = "string[]";
                        else if (rustType.includes("i32") || rustType.includes("f64")) tsType = "number[]";
                        else tsType = "any[]";
                    }
...
`, 'utf8');
    ...
    let generateSqlQueryDisposable = vscode.commands.registerCommand('vella.generateSqlQuery', async () => {
        ...
        if (query) {
            ...
            if (table) { sql = `SELECT * FROM ${table[0]}`; }
            let conditions = [];
            if (q.includes('active')) conditions.push("status = 'active'");
            if (q.includes('over 18')) conditions.push("age > 18");
            if (q.includes('admin')) conditions.push("role = 'admin'");
        ...
        if (doc.languageId === 'rust') {
            ...
            while ((match = erpRegex.exec(text)) !== null) {
                ...
                diagnostics.push(new vscode.Diagnostic(
                    ...
                    "Vella ERP: Unbalanced Double-Entry Ledger Transaction detected.",
                ...
                if (inHotPath) {
                    ...
                    for (const alloc of allocators) {
                        ...
                        while ((allocMatch = alloc.exec(line)) !== null) {
                            diagnostics.push(new vscode.Diagnostic(
                                new vscode.Range(i, allocMatch.index, i, allocMatch.index + allocMatch[0].length),
                                "HFT FATAL: Heap allocation detected in #[hft_hot_path]. This causes jitter. Use &str, array, or Arena allocator.",
                                vscode.DiagnosticSeverity.Error
                            ));
                        }
                    }
    ...
    context.subscriptions.push(
        openAiTunerDisposable,
        syncSdkDisposable,
        networkSocketAnalyzerDisposable,
        networkLatencyProfilerDisposable,
        scaffoldKubernetesDisposable,
        scaffoldTerraformDisposable,
        testApiEndpointDisposable,
        seedDatabaseDisposable,
        nuclearDeterminismCheckDisposable,
        nuclearCoreSimulatorDisposable,
        scadaModbusPingDisposable,
        scadaAnomalyInjectorDisposable,
        f1UdpReplayerDisposable,
        f1EcuCompilerDisposable,
        cryptoToolDisposable,
        scaffoldZkCircuitDisposable,
        web3RpcInspectorDisposable,
        generateWeb3BindingsDisposable,
        generateWalletDisposable,
        openSchemaBuilderDisposable,
        scaffoldReactDisposable,
        scaffoldVueDisposable,
        scaffoldAngularDisposable,
        exportTypesDisposable,
        generateFrontendClientDisposable,
        validateLedgerDisposable,
